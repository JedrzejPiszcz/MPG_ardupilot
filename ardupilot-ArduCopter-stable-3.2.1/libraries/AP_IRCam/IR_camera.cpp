...
extern const AP_HAL::HAL& hal;
 ...
 bool    IRCamera::calculate_midpoint_v2(void) //nowa wersja obliczania pozycji drona, zapamiętuje poprzednią pozycję
 {
    ...
    for (uint8_t i=0; i<=3; i++) // obliczanie dystansu miedzy najodleglejszymi blobami
      ...
      /*  hal.console->print(_last_blob_tab[blob_to_measure_1].x);
        hal.console->print(",");
        hal.console->print(_last_blob_tab[blob_to_measure_1].y);
        hal.console->print("---");
        hal.console->print(_last_blob_tab[blob_to_measure_2].x);
        hal.console->print(",");
        hal.console->print(_last_blob_tab[blob_to_measure_2].y);
        hal.console->println("");
        
        hal.console->print("Pix range: ");
        hal.console->println(_pixel_range);*/
